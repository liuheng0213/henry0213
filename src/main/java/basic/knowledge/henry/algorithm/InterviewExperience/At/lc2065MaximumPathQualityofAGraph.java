...
public class lc2065MaximumPathQualityofAGraph {
    public static void main(String[] args) {
        lc2065MaximumPathQualityofAGraph obj = new lc2065MaximumPathQualityofAGraph();
        int[] values = new int[]{0, 32, 10, 43};
        int[][] edges = new int[][]{{0, 1, 10}, {1, 2, 15}, {0, 3, 10}};
        int i = obj.maximalPathQuality(values, edges, 49);
    ...
    private void dfs(List<int[]>[] adj, int src, int[] values, int maxTime, int[] seen, int[] res, int sum) {
        ...
        for (int[] data : adj[src]) {
            ...
            seen[dst]++;
            sum += (1 == seen[dst] ? values[dst] : 0);// or put  (1 == seen[dst] ? values[dst] : 0) directly in the dfs
            dfs(adj, dst, values, maxTime - t, seen, res, sum);
